...
module.exports = class BlogPostsModel{
  constructor(db){
    this.db = db
  }
  getAll(orderByColumn = 'created_at', orderDirection = 'asc', offset = 0, limit, searchColumns) {
    ...
    return query.then(data => {
      return Promise.all(data.map(post => {
        return this.db('blog_posts_labels').innerJoin('labels', 'labels.id', 'blog_posts_labels.labels_id')
        .select('label_text')
        .where({ blog_posts_id: post.id })
        .then(labels => {
          return {...post, labels: labels.map(({labelText}) => labelText)}
        })
      }))
      .then(posts => {
        if (searchColumns.label) {
  ...
  create(usersId, title, body, labelsIds){
    return this.db('blog_posts')
    .insert({usersId, title, body})
    .returning('*')
    .then(([newBlogPost]) => {
      return Promise.all([
        newBlogPost,
    ...
    .then(([ blogPosts, labels ]) => {
      return blogPosts
    })
  }
  
  remove(id, usersId){
    return this.db('blog_posts').del()
    .where({id, usersId})
    .returning('*')
  }
}
